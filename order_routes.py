...
#funcao para adicionar um item ao pedido
...
#funcao para cancelar um pedido
...
#funcao para ver remover um item
...
async def adicionar_item_pedido(id_item_pedido:int,
    ...
    if not item_pedido:
        raise HTTPException(status_code=400, detail="item no pedido nao existe")
    ...
    return{
        ...
        "quantidade_items_pedido": len(pedido.itens),
        "pedido": pedido
    }

#funcao para lsitar todos os pedidos do sistema, somente admin pode fazer isso
...
#funcao para ver os items de um pedido   
@order_router.get("/pedido/{id_pedido}" )
async def visualizar__pedido(id_pedido: int,session: Session = Depends(pegar_sessao), usuario: Usuario = Depends(verificar_token)):
    pedido = session.query(Pedido).filter(Pedido.id==id_pedido).first()
    if not pedido:
        raise HTTPException(status_code=400, detail="Pedido não encontrado")
    if not usuario.admin and usuario.id != pedido.usuario:
...
#funcao para ver todos os pedidos de um usuario 
